...
_isBlacklistedFile(string &path)
{
  if ((Util::strStartsWith(path, "/dev/") &&
      ...
      Util::strStartsWith(path, "/sys/") ||  // Ignore the cgroup files on Cori
      Util::strStartsWith(path, "/dev/xpmem") ||  // Ignore xpmem on Cori
      Util::strStartsWith(path, "/dev/kgni") ||  // Ignore kgni on Cori
      Util::strStartsWith(path, "/dev/kdreg") ||  // Ignore kdreg on Cori
      Util::strStartsWith(path, "/dev/xpmem") ||  // Ignore xpmem on Cori
...
FileConnection::refill(bool isRestart)
{
  ...
  if (strstr(_path.c_str(), "infiniband/uverbs") ||
      Util::strStartsWith(_path, "/sys/")  || // Ignore cgroup files for Cori
